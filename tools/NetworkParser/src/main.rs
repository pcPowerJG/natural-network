...
mod SimpleNet {
	...
	impl Layer {
		fn get_in_count(&self) -> usize  { in_  }
		fn get_out_count(&self) -> usize { out_ }
		fn get_matrix(&self) -> Vec<Vec<f32>> { matrix }
		fn upd_matrix(&mut self, entered_value: Vec<f32>) {
			//for(int ou =0; ou < out; ou++)
			for ou in 0..self.out_.clone() {

				//for(int hid =0; hid < self.in_.clone(); hid++)
				for hid in 0..self.in_.clone() {
					matrix[hid][ou] += (self.learn_speed.clone() * self.errors[ou].clone() * entered_value[hid]);
				}
				matrix[self.in_.clone()][ou] += (self.learn_speed.clone() * self.errors[ou].clone());
			}
		}
		fn setIO(&mut self, inputs: usize, outputs: usize)
		{
			self.in_= inputs;
			self.out_= outputs;
			self.hidden = Vec::new();
			...
			for inp in 0..self.in_.clone() {
				...
				for out in 0..self.out_.clone(){
					temp_vec.push(0.0); //# (rand() / RAND_MAX) - 0.5) * out.powf(-0.5))
				}
